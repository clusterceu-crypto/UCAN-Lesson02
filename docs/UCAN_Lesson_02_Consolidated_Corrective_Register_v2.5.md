...
| L02-COR-007 | Localization | `index.html`, `js/script.js` | AI / ШІ terminology | Learner-facing text mixed `AI`, `ШІ`, `запит` and `prompt`. | Learner-facing interface uses `ШІ-підтримка` and `промпт`; product names remain official. | Minor | Inventory contains no unjustified learner-facing AI selector wording. | PASS | Localization Rules |
...
| L02-COR-009 | Practical UX | `index.html` | Step 4 instruction | Mentioned print and mixed `AI-помічник`, although export is direct PDF download. | Explains save, PDF download and optional ШІ verification. | Minor | Text matches actual actions. | PASS | Learner-facing Localization Rules |
